...
filter_sizes = [3,4,5]	# filter/kernal size (in terms of number of words)
...
for i, filter_size in enumerate(filter_sizes):
	...
	W_conv = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W_conv")	# ( 3, 50, 1, 70 )
...
h_pool = tf.concat(3, pooled_outputs)	# (N, 1, 1, 210)
p1 = tf.squeeze(h_pool)			# (N, 210)	
num_filters_total = num_filters * len(filter_sizes)

#Fully connected layer operations
W_ff = tf.Variable(tf.truncated_normal([num_filters_total, num_classes], stddev=0.1), name="W") # (210, 3)
...
with tf.Session() as sess :
	...
	# Create Dummy Dataset
 	D = np.asarray( np.random.randint(0, high=word_dict_size, size = [1000, sentMax] ), dtype='int32')		# 1000 : size of data set
